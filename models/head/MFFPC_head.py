...
class MFFPCHead(nn.Module):
    ...
    def build_from_config(config, **kwargs):
        ...
        try:
            ...
            return MFFPCHead(conv, blocks, final, **kwargs)
        except:
            return MFFPCHead(conv, None, final, **kwargs)

        
def MFFPC_head(config, **kwargs):
    head_config = json.load(open(config, 'r'))['head']
    head = MFFPCHead.build_from_config(head_config, **kwargs)
    return head