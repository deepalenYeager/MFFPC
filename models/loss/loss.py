...
class EmbLoss(nn.Module):
    ...
    def forward_single(self, emb, instance, kernel, training_mask):
        ...
        max_distance = emb.new_zeros(num_instance, dtype=torch.float32)  # 最大距离
        ...
        for i, lb in enumerate(unique_labels):
            ...
            max_distance[i] = torch.max(dist)  # 找到最大距离

        # 限制最大距离，避免数值不稳定
        epsilon = 1e-6
        max_distance = torch.clamp(max_distance, min=epsilon)

        l_agg = emb.new_zeros(num_instance, dtype=torch.float32)
        l_cos = emb.new_zeros(num_instance, dtype=torch.float32)
        ...
        for i, lb in enumerate(unique_labels):
            ...
            emb_ = emb[:, ind]
            dist = (emb_ - emb_mean[:, i:i + 1]).norm(p=2, dim=0)
            weighted_dist = dist / max_distance[i]
            # 直接使用加权距离计算损失
            l_agg[i] = torch.mean(weighted_dist)

            #余弦相似度损失
            cos_sim = F.cosine_similarity(emb_.T, emb_mean[:, i].unsqueeze(0).repeat(emb_.shape[1], 1), dim=1)
            l_cos[i] = 1 - torch.mean(cos_sim)  # 方向一致性优化

        l_agg = torch.mean(l_agg[1:])
        l_cos = torch.mean(l_cos[1:])
